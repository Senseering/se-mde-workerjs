...
/**
 * Is used to register at an edge node with the necessary options given. It checks if the worker is
 * registered and registers it automatically if not.
 *
 * @param {String} edge URL (without Port, see port instead) to reach the edge node
 * @param {String} completeEdgeLink Url + port of the edge node
...
function Component(params) {
    ...
    if (fs.existsSync('./config/development.json')) {
        ...
        if (config.get('apiDomain') === '') {
            throw new Error('No edgenode ip used')
        }
...
/**
 * Init should register the worker at the edge node
 * or load all required data like id, keys and templates
 */
Component.prototype.init = async function (run) {
    ...
    while (socket.readyState !== 1) {
        ...
        debug('Waiting for connection to edgenode')
    ...
    await register.submit(this.completeEdgeLink + "/core/", {
        schema: schema,
        info: info,
 ...
 */
Component.prototype.run = async function (datainput, params, price = undefined) {
